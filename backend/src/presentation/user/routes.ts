...
import { JwtAdapter } from '@src/config/jwt/jwt.adapter';
...
export class UserRoutes {
  static get routes(): Router {
    ...
    router.get( endpoints.root, JwtAdapter.authenticateToken, JwtAdapter.validateRoles, controller.findAll );
    router.put( endpoints.id, controller.update );
    router.post( endpoints.user.login, controller.login );
    return router;
  }
}